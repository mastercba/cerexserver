<?php 
	...
	//verifica si ya existe el tds para esa fecha y ese tag...
	$result2 = mysql_query("SELECT * FROM phtdsm1", $conexion);	
	while($row2 = mysql_fetch_array($result2)){
		$t_leido = $row2['tag'];
		$f_leido = $row2['fecha'];
			if($tag_mad == $t_leido AND $cfinal == $f_leido){
				mysql_query("UPDATE phtdsm1 SET tdsam='".$tds_number."' WHERE fecha = '".$cfinal."'");
				$flag1 = 1;
			}else{}
			if($tag_mad != $t_leido AND $cfinal != $f_leido){
			}else{}	
	...
	if($flag1 == 0){
		...
		VALUES('$tag_mad','$cfinal','0','".$tds_number."')"), $conexion);
	}
