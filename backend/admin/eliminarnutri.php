<?php 
	...
	$idnutriente = $_GET['nutriid'];

    $once = 74; 
    //Recupero el monto a sumar y restar
        $result7 = mysql_query("SELECT * FROM nutriente WHERE id='".$idnutriente."'", $conexion);    
    ...
    if($monto == 0){
    //Borro fila de agua         
        $result = mysql_query("DELETE FROM nutriente WHERE id='".$idnutriente."'", $conexion);
