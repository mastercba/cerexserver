<?php 
	...
	while($row = mysql_fetch_array($result4)){
			...
			//$afinal =	$row40['afinal'];	
			for ($x=1; $x<=5; $x++) {
				$be="a"; if($x==1){$flag = $x;}else{$flag = $x - 1;}
				$xe="$x"; $y="$flag";
				$a = $be.$xe; $ay= $be.$y;
				$res = $row[$a]; $yres = $row[$ay];
				$date = $row['afinal'];
				$dateInit = $row['ainicio'];
				if($res == 0 AND $out == 0 AND $a != 'a1'){
				...
				if($res == 1 AND $out == 0 AND $a == 'a5'){
					mysql_query("UPDATE $modulo SET $a='0' WHERE id = '1'");
				...
				if($res == 0 AND $out == 0 AND $a == 'a1'){
					...
					$afinal = strtotime ( '-'.$inc.' day' , strtotime ( $date ) ) ;
					$afinal = date ( 'Y-m-j' , $afinal );
					mysql_query("UPDATE $modulo SET afinal= '".$dateInit."' WHERE id = '1'");
					$out = 1;	
				}	
			}
	}		
